...
const reducer = (state = initialState, action: { type: Actions; newText: string; taskId: string }) => {
  switch (action.type) {
    ...
    case Actions.ADD_TASK:
      ...
      if (state.newTaskText === undefined || state.newTaskText.length === 0) break
    ...
    case Actions.DELETE_TASK:
      return {
        ...state,
        allTaskList: state.allTaskList.filter(task => task.id !== action.taskId),
      }
    case Actions.CLICK_CHECKBOX:
      const index = state.allTaskList.findIndex(listItem => listItem.id === action.taskId)
      const target = state.allTaskList[index]
      return {
        ...state,
        allTaskList: Object.assign([], state.allTaskList, { [index]: { ...target, done: !target.done } }),
      }
